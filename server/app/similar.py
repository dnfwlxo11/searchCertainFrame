...
def getImageDB_li(dirName):
    imageDB_li = os.listdir('static_bac/{}/{}/'.format(dirName, dirName))
    imageDB_li = ['static_bac/{}/{}/'.format(dirName, dirName) + image for image in imageDB_li]
...
def setImageDB(imageDB_li, dirName):
    if not os.path.exists(dirName + '_npy'):
        os.mkdir('static_bac/{}/{}_npy'.format(dirName, dirName))
    for img in sorted(imageDB_li):
        feature = feature_extractor(model, img)

        feature_path = 'static_bac/{}/{}_npy/'.format(dirName, dirName) + img.split('/')[-1].split('.')[0] + '.npy'
...
def speedMode(dirName, src, len):
    ...
    features = []
    img_path = []
    for img_npy in Path('static_bac/{}/{}_npy/'.format(dirName, dirName)).glob('*.npy'):
        features.append(np.load(img_npy))
        img_path.append(os.path.join('static_bac/{}/{}/'.format(dirName, dirName), img_npy.stem + '.jpg'))
...
def calc_cossim(features, img_path, src):
    ...
    scores = [(result[id], img_path[id].split('/')[3]) for id in ids]
